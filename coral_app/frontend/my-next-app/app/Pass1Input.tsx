...
// ── Storage ───────────────────────────────────────────────────────────────────
const SK = {
  models: "coral_p1_models", sourceId: "coral_p1_sourceId", alignInfo: "coral_p1_alignInfo",
  mapping: "coral_p1_mapping", mappingConfirmed: "coral_p1_mappingConfirmed",
  rows: "coral_p1_rows", rowIdx: "coral_p1_rowIdx", fileName: "coral_p1_fileName",
};
...
const ZONE_META: Record<ColRole, { label: string; hint: string; border: string; chip: string }> = {
...
const s = {
  ...
  keyInput:   "rounded border border-zinc-700 bg-zinc-900 px-2 py-0.5 text-xs font-mono text-zinc-200 placeholder-zinc-600 focus:border-cyan-700 focus:outline-none",
  select:     "flex-1 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs font-mono text-zinc-200 focus:border-cyan-700 focus:outline-none",
  ...
  btnConfirm: "w-full rounded-lg border border-cyan-800 bg-cyan-950 py-2.5 text-xs font-mono font-semibold text-cyan-300 uppercase tracking-widest hover:bg-cyan-900 hover:border-cyan-600 transition-colors",
  ...
  btnGhost:   "px-2.5 py-1 rounded border border-zinc-700 text-xs font-mono text-zinc-500 hover:border-zinc-500 hover:text-zinc-300 transition-colors",
  ...
  btnSrc:     (on: boolean) => on
    ? "px-2.5 py-1 rounded text-xs font-mono bg-cyan-900 text-cyan-300 border border-cyan-700 transition-colors"
  ...
  delimBtn:   (on: boolean) => on
    ? "px-3 py-1 rounded text-xs font-mono border transition-colors border-cyan-700 bg-cyan-950 text-cyan-300"
    : "px-3 py-1 rounded text-xs font-mono border transition-colors border-zinc-700 bg-zinc-900 text-zinc-500 hover:border-zinc-500 hover:text-zinc-300",
  modelCard:  (isSrc: boolean) => `rounded-xl border p-3 space-y-2 transition-colors ${isSrc ? "border-cyan-800 bg-cyan-950/20" : "border-zinc-800 bg-zinc-950"}`,
  chip:       (chipCls: string, dragging: boolean) => `flex items-center gap-1.5 px-2 py-1 rounded border text-xs font-mono cursor-grab active:cursor-grabbing select-none transition-opacity ${chipCls} ${dragging ? "opacity-30" : ""}`,
};
...
function buildDefaultMapping(cols: string[]): ColMapping[] {
  return cols.map(col => ({ col, role: "exclude" as ColRole, modelKey: col }));
}
...
export default function Pass1Input({ onAligned }: Props) {
  ...
  // Hydrate from localStorage on mount — restores display state only
  useEffect(() => {
    const m  = lsGet<Model[]>(SK.models);       const si = lsGet<string>(SK.sourceId);
    const ai = lsGet<AlignInfo>(SK.alignInfo);  const mp = lsGet<ColMapping[]>(SK.mapping);
    const mc = lsGet<boolean>(SK.mappingConfirmed); const rs = lsGet<RawRow[]>(SK.rows);
    const ri = lsGet<number>(SK.rowIdx);        const fn = lsGet<string>(SK.fileName);
    if (m)           setModels(m);
  ...
  // Persist each piece of state to localStorage when it changes
  ...
  // Re-parse when delimiter changes (DSV only)
  useEffect(() => {
    if (!rawContent || isJson) return;
    const { headers: h, rows: r } = parseDSV(rawContent, delim);
    setHeaders(h); setRows(r); setMapping(buildDefaultMapping(h)); setMappingConfirmed(false);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [delim]);

  function loadRowIntoModels(rowList: RawRow[], idx: number, mp: ColMapping[]) {
    const next = mp.filter(c => c.role === "model").map(c => mkModel(c.modelKey || c.col, rowList[idx][c.col] ?? ""));
    setModels(next); setSourceId(next[0]?.id ?? ""); setAlignInfo(null);
  }

  function handleFile(file: File) {
    setError(null); setMappingConfirmed(false); setMapping(null); setAlignInfo(null); setFileName(file.name);
    ...
    reader.onload = e => {
      ...
      try {
        if (file.name.toLowerCase().endsWith(".json")) {
          ...
          setIsJson(false); setRawContent(content);
          const detected = sniffDelimiter(content.split(/\r?\n/)[0] ?? "");
          setDelim(detected);
  ...
  function setColRole(col: string, role: ColRole) {
  ...
  function confirmMapping() {
    ...
    setMappingConfirmed(true); loadRowIntoModels(rows, rowIdx, mapping);
  }

  const addModel    = () => setModels(p => [...p, mkModel()]);
  const removeModel = (id: string) => setModels(p => {
  ...
  function handleRowChange(idx: number) {
    setRowIdx(idx); if (mapping) loadRowIntoModels(rows, idx, mapping); setAlignInfo(null);
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setAlignInfo(result); onAligned(result);
    } catch (e: unknown) { setError(e instanceof Error ? e.message : "Alignment failed"); }
    finally { setLoading(false); }
  }

  function handleDrop(role: ColRole, e: React.DragEvent) {
    e.preventDefault();
    ...
    <div className="space-y-6">
      ...
      {!fileName ? (
        ...
        >
          ...
          <p className={`${s.label} mb-2`}>Drop file or click to browse</p>
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 px-4 py-3 flex items-center gap-3">
          ...
          <span className={s.labelDim}>{rows.length} rows</span>
          {mappingConfirmed && (
            <button onClick={() => setMappingConfirmed(false)} className={s.btnGhost}>↩ Re-map</button>
          )}
          <button onClick={() => {
            setFileName(""); setHeaders([]); setRows([]); setMapping(null);
            setMappingConfirmed(false); setAlignInfo(null); setRawContent(null);
          }} className={s.btnDelete}>✕</button>
          <input ref={fileRef} type="file" accept=".tsv,.csv,.json" className="hidden"
            onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f); }} />
        </div>
      )}
      ...
      {fileName && !isJson && !mappingConfirmed && (
        <div className="flex items-center gap-2">
          <span className={`${s.label} shrink-0`}>Delimiter</span>
          <div className="flex gap-1">
            {["\t", ",", ";", "|"].map(d => (
              <button key={d} onClick={() => setDelim(d)} className={s.delimBtn(delim === d)}>
                {DELIM_LABEL[d]}
              </button>
            ))}
          </div>
          <span className={`${s.labelDim} ml-1`}>auto-detected</span>
        </div>
      )}
      ...
      {mapping && !mappingConfirmed && (
        <div className="rounded-xl border border-zinc-700 bg-zinc-950 overflow-hidden">
          <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
            <p className={s.label}>Map columns</p>
            <span className={s.labelDim}>{headers.length} columns · drag to assign</span>
          </div>
            ...
            <table className="text-xs font-mono min-w-full">
              <thead>
                <tr className="border-b border-zinc-800">
                  {headers.map(h => <th key={h} className="px-3 py-2 text-left text-zinc-500 font-normal whitespace-nowrap">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {rows.slice(0, 3).map((row, i) => (
                  <tr key={i} className="border-b border-zinc-900">
                    {headers.map(h => (
                      <td key={h} className="px-3 py-1.5 text-zinc-500 max-w-[200px] truncate font-urdu text-right" dir="rtl">{row[h]}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
          ...
          <div className="p-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
            {(["label", "model", "exclude"] as ColRole[]).map(role => {
              ...
              const m    = ZONE_META[role];
              return (
                <div key={role}
                  onDragOver={e => e.preventDefault()} onDrop={e => handleDrop(role, e)}
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <p className={s.labelDim}>{m.hint}</p>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {zone.map(c => (
                      <div key={c.col} className="flex flex-col gap-1">
                        <div draggable onDragStart={() => setDragging(c.col)} className={s.chip(m.chip, dragging === c.col)}>
                          <span className="text-zinc-600">⠿</span>{c.col}
                        </div>
                        {role === "model" && (
                          <input type="text" value={c.modelKey} placeholder="key name..."
                            onChange={e => setMapping(p => p ? p.map(x => x.col === c.col ? { ...x, modelKey: e.target.value } : x) : p)}
                            onClick={e => e.stopPropagation()} className={s.keyInput} />
                        )}
                      </div>
                    ))}
                    {zone.length === 0 && <p className={`${s.labelDim} italic mt-1`}>drop here</p>}
                  </div>
                </div>
              );
            })}
          ...
          <div className="px-4 pb-4">
            <button onClick={confirmMapping} className={s.btnConfirm}>CONFIRM MAPPING →</button>
          </div>
        </div>
      )}
      ...
      {mappingConfirmed && rows.length > 1 && (
        <div className={s.row}>
          <label className={`${s.label} shrink-0`}>Row</label>
          <select value={rowIdx} onChange={e => handleRowChange(Number(e.target.value))} className={s.select}>
            {rows.map((r, i) => (
              <option key={i} value={i}>{i + 1}. {labelCol ? (r[labelCol] ?? `Row ${i + 1}`) : `Row ${i + 1}`}</option>
            ))}
          </select>
        </div>
      )}
      ...
      {(mappingConfirmed || !fileName) && (
        <>
          <div className="flex items-center justify-between">
            <p className={s.label}>{models.length} models</p>
            <button onClick={addModel} className={s.btnSm}>+ Add model</button>
          </div>
          <div className="space-y-2">
            {models.map(m => {
              const isSrc = m.id === sourceId;
              return (
                <div key={m.id} className={s.modelCard(isSrc)}>
                  <div className="flex items-center gap-2">
                    <input type="text" value={m.key} onChange={e => updateModel(m.id, { key: e.target.value })}
                      placeholder="model key..." className={s.textInput} />
                    <div className="flex-1" />
                    <button onClick={() => setSourceId(m.id)} className={s.btnSrc(isSrc)}>
                      {isSrc ? "★ SRC" : "SRC"}
                    </button>
                    {models.length > 1 && (
                      <button onClick={() => removeModel(m.id)} className={s.btnDelete}>✕</button>
                    )}
                  </div>
                  <textarea dir="rtl" rows={2} value={m.text} onChange={e => updateModel(m.id, { text: e.target.value })}
      ...
      {(mappingConfirmed || !fileName) && (
        <button onClick={handleSubmit} disabled={loading || !models.length} className={s.btnPrimary}>
