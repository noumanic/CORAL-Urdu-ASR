...
function buildPrompt(alignInfo: AlignInfo, oovResult: OOVResult): string {
  ...
  const sourceAlign = alignInfo[sourceModel] as { normalized_attempt?: string[] };
  const sourceText  = sourceAlign?.normalized_attempt?.join(" ") ?? "";

  const modelOutputs: Record<string, string> = {};
  Object.entries(alignInfo).forEach(([key, val]) => {
    if (key === "source_model") return;
    const m = val as { aligned_attempt?: string[] };
    if (m?.aligned_attempt) modelOutputs[key] = m.aligned_attempt.join(" ");
  });

  const oovSummary = Object.entries(oovResult.metadata)
    .map(([word, candidates]) => {
      const top = Object.keys(candidates)[0] ?? null;
      return top ? `"${word}" -> suggested: "${top}"` : `"${word}" (no candidate)`;
    }).join("\n");

  return `You are an expert Urdu ASR post-correction system. You will receive:
1. A source Urdu transcript (already normalised)
2. Outputs from multiple ASR models for the same audio
3. A list of Out-Of-Vocabulary (OOV) words detected with suggested corrections

Your job is to produce the single best corrected Urdu transcript by:
- Using majority voting across model outputs where models agree
- Preferring OOV candidate suggestions for flagged unknown words
- Using your Urdu linguistic knowledge to resolve conflicts

SOURCE TRANSCRIPT (model: ${sourceModel}):
${sourceText}

ALL MODEL OUTPUTS:
${Object.entries(modelOutputs).map(([m, t]) => `[${m}]: ${t}`).join("\n")}

OOV WORDS DETECTED (with top candidates):
${oovSummary || "(none)"}

Respond ONLY with valid JSON in this exact structure, no markdown fences:
{
  "corrected": "<the final corrected Urdu transcript as a single string>",
  "reasoning": "<1-2 sentence explanation of your approach>",
  "changes": [
    { "original": "<original word>", "corrected": "<corrected word>", "reason": "<why>" }
  ]
}

Rules:
- Keep the corrected text in Urdu script
- Only include words in "changes" that were actually modified
- If no corrections needed, return empty array for changes and the source text for corrected`;
}
...
export default function Pass3Correction({ alignInfo, oovResult }: Props) {
  ...
  async function runLLM() {
    ...
    try {
      ...
      const prompt = buildPrompt(alignInfo, oovResult);
      setLlmPhase("waiting");
        ...
        {!loading && mode === "llm" && llmResult && (
          <>
            <span className={`flex items-center gap-2 font-mono text-xs ${isOR ? "text-orange-400" : "text-violet-400"}`}>
              ...
              {isOR ? modelLabel(selectedModel) : modelLabel(selectedModel)}
            </span>
