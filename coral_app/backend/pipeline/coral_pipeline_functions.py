...
def asr_aligner(ensemble, source_model, weight_func=levenshtein, deletion_cost=1.0, insertion_cost=1.0):
    ...
    for model_name in model_name_list:
        ...
        align_info[model_name]['aligned_attempt'] = alignment_list
        align_info[model_name]['aligned_info'] = alignment_type
        source_string = ' '.join(align_info[source_model]['normalized_attempt'])
        target_string = ' '.join(align_info[model_name]['normalized_attempt'])
        split_merge_alignment_result = generate_alignment_data(source_string,target_string)
        align_info[model_name]['split_merge_aligned_attempt'] = split_merge_alignment_result['aligned_output']
        align_info[model_name]['split_merge_metadata'] = split_merge_alignment_result['metadata']
        align_info[model_name]['split_merge_metadata_base'] = split_merge_alignment_result['metadata_base']
        align_info[model_name]['split_merge_aligned_info'] = split_merge_alignment_result['info']
    return align_info
...
def apply_corrections(align_info, oov_metadata):
    ...
    source_words = align_info[source]['split_merge_aligned_attempt']
    n            = len(source_words)
    corrected    = []
    ...
    for i in range(n):
        ...
        votes = Counter()
        for model in model_names:
            words     = align_info[model]['split_merge_aligned_attempt']
            matchinfo = align_info[model]['split_merge_aligned_info']
